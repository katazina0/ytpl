...
export function parseBody(body, options = {}) {
  const json = tryParseBetween(body, "var ytInitialData = ", "};", true) ||
...
export function parseNumFromText(txt) {
  return Number(parseText(txt).replace(/\D+/g, ""));
}

// Request Utility
export function doPost(url, opts, payload) {
  if (!opts) opts = {};
...
export function checkArgs(plistId, options = {}) {
  // Validation
  if (!plistId) {
